...
def test_comment_box_props_with_object_and_user(an_user, an_article):
    ...
    assert props['list_url'] == reverse('comments-xtd-api-list',
                                        kwargs={'content_type': ctype_slug,
                                                'object_pk': an_article.id})
    assert props['count_url'] == reverse('comments-xtd-api-count',
                                         kwargs={'content_type': ctype_slug,
                                                 'object_pk': an_article.id})
    ...
    reactions_overlays = json.dumps(settings.COMMENTS_XTD_REACTIONS_JS_OVERLAYS)
...
@pytest.mark.django_db
def test_comment_box_props_with_new_app_model_options(an_user, an_article,
                                                      monkeypatch):
    """
...
@pytest.mark.django_db
def test_comment_box_props_with_new_max_thread_level(an_user, an_article,
                                                     monkeypatch):
