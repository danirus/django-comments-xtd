...
def thread_test_step_1(article, model=None, **kwargs):
    model = model if model else get_model()
...
def thread_test_step_2(article, model=None, **kwargs):
    model = model if model else get_model()
    article_ct = ContentType.objects.get(app_label="tests", model="article")
    site = Site.objects.get(pk=1)
...
class PublishOrUnpublishNestedCommentsTwoTestCase(ArticleBaseTestCase):
    ...
    @patch.multiple(
        "django_comments_xtd.conf.settings", COMMENTS_XTD_MODEL=_model
    )
    def test_removing_c1_unpublishes_c3_and_c4(self):
