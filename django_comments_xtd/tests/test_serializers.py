...
def _create_user(can_moderate=False):
    bob = User.objects.create_user(
        "joe", "fulanito@detal.com", "pwd", first_name="Joe", last_name="Bloggs"
    )
    if can_moderate:
        ...
        permission = Permission.objects.get(
            content_type=ct, codename="can_moderate"
        )
...
get_fake_avatar_function_path = (
    "django_comments_xtd.tests.test_serializers.get_fake_avatar"
)
...
class ReadCommentsGetUserAvatarTestCase(TestCase):
    ...
    @patch.multiple(
        ...
        COMMENTS_XTD_API_GET_USER_AVATAR=get_fake_avatar_function_path,
    )
