...
def send_confirm_user_reset_password_request(
    email,
    key,
    site,
    scheme,
    text_tmpl="users/user_reset_password_confirmation.txt",
    html_tmpl="users/user_reset_password_confirmation.html",
):
    subject = _("confirm reset password request")
    confirmation_url = reverse(
        "reset-password-confirm", args=[key.decode("utf-8")]
    )
    context = {
        "email": email,
        "confirmation_url": confirmation_url,
